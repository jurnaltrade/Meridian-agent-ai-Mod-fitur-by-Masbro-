...
import { openHedge, closeHedge, getHedgeStatus } from "./hedge-overlay.js";
import { pickArm, recordArmOutcome, getBanditSummary } from "../bandit-allocator.js";
import { scanWhaleActivity, getWhaleSignalForPool } from "../whale-shadow.js";
import { getInsuranceStatus, insuranceWithdraw, contributeFromClaim } from "../profit-insurance.js";
import { getPortfolioGreeks, getPortfolioHedgeSizing } from "../portfolio-greeks.js";
import { registerJitWatch, checkJitOpportunity, closeExpiredJitSessions, getJitSummary } from "../jit-liquidity.js";
...
const toolMap = {
  ...
  // ─── Advanced modules ──────────────────────
  get_volatility_regime: getVolatilityRegime,
  pick_bandit_arm: pickArm,
  get_bandit_summary: getBanditSummary,
  scan_whale_activity: scanWhaleActivity,
  get_whale_signal_for_pool: getWhaleSignalForPool,
  get_insurance_status: getInsuranceStatus,
  insurance_withdraw: insuranceWithdraw,
  get_portfolio_greeks: getPortfolioGreeks,
  get_portfolio_hedge_sizing: getPortfolioHedgeSizing,
  register_jit_watch: registerJitWatch,
  check_jit_opportunity: checkJitOpportunity,
  close_expired_jit_sessions: closeExpiredJitSessions,
  get_jit_summary: getJitSummary,
};
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        // Auto-open a hedge for this position (no-op unless config.hedge.enabled + venue configured)
        openHedge({
          position_address: result.position,
          pool_address: args.pool_address,
          base_symbol: result.base_symbol || args.pool_name,
          position_notional_usd: result.position_notional_usd ?? (args.amount_y ? args.amount_y * (result.token_y_price_usd || 0) : null),
        }).catch(() => {});
      } else if (name === "close_position") {
        notifyClose({ pair: result.pool_name || args.position_address?.slice(0, 8), pnlUsd: result.pnl_usd ?? 0, pnlPct: result.pnl_pct ?? 0 }).catch(() => {});
        // Auto-close matching hedge, if any
        closeHedge({ position_address: args.position_address }).catch(() => {});
        // Record bandit outcome if this position was arm-tagged (via set_position_note at deploy time)
        if (result.arm_id || args.arm_id) {
          recordArmOutcome({ arm_id: result.arm_id || args.arm_id, pnl_usd: result.pnl_usd });
        }
        ...
        const { result: claimSwapResult } = await swapBaseToSolWithRetry(result.base_mint, "after claim");
        // Skim a % of the resulting SOL into the profit-insurance buffer.
        // (Only fires when autoSwapAfterClaim is on, since that's the only path
        // where we get a concrete SOL amount_out to compute the reserve from.)
        if (claimSwapResult?.amount_out) {
          contributeFromClaim({
            position_address: args.position_address,
            pool_name: result.pool_name,
            amount_sol: claimSwapResult.amount_out,
          }).catch(() => {});
        }
      }
    }
