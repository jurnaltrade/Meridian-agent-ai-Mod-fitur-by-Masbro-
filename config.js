...
export const config = {
  ...
  // ─── Volatility Regime (adaptive bin/strategy sizing) ─
  volatilityRegime: {
    lowMaxPct:            u.volLowMaxPct        ?? 1.5,
    mediumMaxPct:         u.volMediumMaxPct     ?? 4,
    highMaxPct:           u.volHighMaxPct       ?? 9,
    extremePriceSwingPct: u.volExtremeSwingPct  ?? 25,
  },

  // ─── IL Hedge Overlay (OFF by default — requires _venue wiring) ─
  hedge: {
    enabled:               u.hedgeEnabled          ?? false,
    hedgeRatio:            u.hedgeRatio            ?? 0.5,
    maxHedgeNotionalUsd:   u.hedgeMaxNotionalUsd   ?? 200,
    minPositionUsdToHedge: u.hedgeMinPositionUsd   ?? 50,
    venue:                 u.hedgeVenue            ?? "none",
  },

  // ─── Bandit Allocator (Thompson Sampling over strategy presets) ─
  banditArms: u.banditArms ?? null, // null = use built-in defaults in bandit-allocator.js

  // ─── Profit Insurance Buffer ───────────
  insurance: {
    enabled:        u.insuranceEnabled        ?? true,
    reservePct:     u.insuranceReservePct     ?? 15,
    walletAddress:  u.insuranceWalletAddress  ?? null,
    maxBufferSol:   u.insuranceMaxBufferSol   ?? null,
  },

  // ─── JIT / Flow-Following Rapid Liquidity (OFF by default) ─
  jit: {
    enabled:                u.jitEnabled               ?? false,
    burstVolumeMultiplier:  u.jitBurstMultiplier        ?? 3,
    pollIntervalSec:        u.jitPollIntervalSec        ?? 15,
    holdSeconds:            u.jitHoldSeconds            ?? 90,
    positionSizeSol:        u.jitPositionSizeSol        ?? 0.1,
    maxConcurrentSessions:  u.jitMaxConcurrentSessions  ?? 1,
    tightBinsBelow:         u.jitTightBinsBelow         ?? 6,
    tightBinsAbove:         u.jitTightBinsAbove         ?? 6,
    minFeeToJustifyGasUsd:  u.jitMinFeeUsd              ?? 0.5,
  },
