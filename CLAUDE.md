...
| `tools/rugcheck.js` | ~135 | `getRugcheckSignals({mint})` — fetches rugcheck.xyz report, derives `conviction_score` (100−normalized risk), `suspicious_pct` (insider holders), `sniper_pct`/`bundle_pct` (insider networks), `top10_pct`. 15-min cache, 10s timeout, fail-open nulls. Internal screening helper (not an LLM tool). Powers the four holder-quality screening filters in `index.js`. |
...
4. **Hard filters after recon**: launchpad allow/block, `bot_holders_pct > maxBotHoldersPct`, and rugcheck-derived holder-quality (`maxSniperPct`, `maxSuspiciousPct`, `maxBundlePct`, `convictionScoreMin` — see `tools/rugcheck.js`). Rugcheck is only fetched when at least one of those four thresholds is set, and each check fails open when rugcheck doesn't supply the metric.
...
| `screening` | `excludeHighSupplyConcentration`, `minFeeActiveTvlRatio`, `minTvl`, `maxTvl`, `minVolume`, `minOrganic`, `minQuoteOrganic`, `minHolders`, `minMcap`, `maxMcap`, `minBinStep`, `maxBinStep`, `timeframe`, `category`, `minTokenFeesSol`, `useDiscordSignals`, `discordSignalMode`, `avoidPvpSymbols`, `blockPvpSymbols`, `maxBotHoldersPct`, `maxTop10Pct`, `maxSniperPct`, `maxSuspiciousPct`, `maxBundlePct`, `convictionScoreMin`, `allowedLaunchpads`, `blockedLaunchpads`, `minTokenAgeHours`, `maxTokenAgeHours` | see `user-config.example.json` |
